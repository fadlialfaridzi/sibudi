...
exports.login = async (req, res) => {
  try {
    ...
    if (memberRows.length === 0) {
      // not a member → try user (pustakawan)
      ...
      return res.status(200).redirect('/inside/peminjaman'); // keep your original path (/peminjaman or /inside/peminjaman)
    }
    ...
    return res.status(200).redirect('/outside/detailPinjam'); // your detail route
  } catch (err) {
...
exports.logout = (req, res) => {
  req.session.destroy(() => {
    res.clearCookie('sibudi_session_id'); // or 'connect.sid' depending on your session key
    res.redirect('/login');
  });
};
